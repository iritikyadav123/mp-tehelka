...
export default function StoryPage({className} : {className: string}) {
    ...
    return <div className={cn("", className)}>
        <div style={{ backgroundImage: `url('/pic2.webp')`}} className="bg-center bg-cover h-[70%] sm:h-[80%] w-full items-end rounded-lg flex  justify-center "> 
        <div className={`text-gray-900 text-2xl sm:text-3xl font-semibold w-[95%] pb-5 mt-8  bg-white/20 rounded-xl pl-2 backdrop-blur-2xl`}>{contentData.title}</div>
        </div>

       
        <div className={`text-gray-800 h-auto  text-xl mt-5 first-letter:text-5xl w-[95%]  overflow-auto `}>  {contentData.description.substring(0,content)} 
         {
